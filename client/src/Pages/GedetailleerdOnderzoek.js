...
export default function GedetaileerdOnderzoek() {
  ...
  const [volleBeschrijving, setVolleBeschrijving] = useState(
    "Elden Ring is an upcoming action role-playing game developed by FromSoftware and published by Bandai Namco Entertainment. The game is a collaborative effort between game director Hidetaka Miyazaki and fantasy novelist George R. R. Martin. It is scheduled for release for Microsoft Windows, PlayStation 4, PlayStation 5, Xbox One, and Xbox Series X/S on 21 January 2022."
  );
  ...
  //zoek naar juiste deelname aantal op basis van onderzoek id
  const aantalDeelnames = countData?.find(
    (onderzoek) => onderzoek.$id === onderzoekId
  );
  ...
  const fetchOnderzoeken = async () => {
    const response = await fetch(
      "https://localhost:7225/api/Onderzoek/GetOnderzoekByID/" + onderzoekId
    );
    ...
    setLocatie(
      data.soortOnderzoek === "Enquete" ? "Online" : data.soortOnderzoek
    );
  ...
  const handleAanmelden = async (e) => {
    await fetch(
      "https://localhost:7225/api/Onderzoek/JoinErvaringsdeskundigeToOnderzoek",
      {
        method: "POST",
        headers: {
      ...
      if (response.status === 405) {
        ...
        if (response.status === 500) {
          alert("U bent al aangemeld voor dit onderzoek");
        } else {
          ...
          <div id={styles.blok_1} name="blok_1">
            ...
            <div id={styles.blok_2}>
              ...
              <p id={styles.text5}>Doelgroep: {doelgroep}</p>
                {aantalDeelnames ? (
                  <p>Aantal deelnemers: {aantalDeelnames.count}</p>
                ) : (
                  <p>unknown</p>
                )}
